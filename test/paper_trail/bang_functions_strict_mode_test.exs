defmodule PaperTrailTest.StrictModeBangFunctions do
  ...
  test "[multi tenant] updating a company creates a company version with correct item_changes" do
    ...
    inserted_company_version = PaperTrail.get_version(inserted_company, prefix: tenant)
      |> serialize
    updated_company = update_company_with_version_multi(
      inserted_company, @update_company_params, originator: user
    )
  ...
  test "[multi tenant] updating a person creates a person version with correct attributes" do
    ...
    inserted_initial_company = create_company_with_version_multi(%{
      name: "Acme LLC", website: "http://www.acme.com"
    })
    inserted_target_company = create_company_with_version_multi(%{
      name: "Another Company Corp.", is_active: true, address: "Sesame street 100/3, 101010"
    })
    inserted_person = Person.changeset(%Person{}, %{
      first_name: "Izel",
    ...
    inserted_person_version = PaperTrail.get_version(inserted_person, prefix: tenant)
      |> serialize
    updated_person = Person.changeset(inserted_person, %{
      first_name: "Isaac",
      visit_count: 10,
  ...
  test "[multi tenant] deleting a person creates a person version with correct attributes" do
    ...
    deleted_person = PaperTrail.delete!(
      updated_person, origin: "admin", meta: %{linkname: "izelnakri"}, prefix: tenant
    )
    deleted_person_version = PaperTrail.get_version(deleted_person, prefix: tenant)
      |> serialize
  ...
  defp update_company_with_version_multi(company, params \\ @update_company_params, options \\ nil) do
    opts_with_prefix = Keyword.put(options || [], :prefix, MultiTenant.tenant())
